...
export const NegociosSectors: React.FC = () => {
  ...
  return (
    <Section spacing="comfortable" className="relative bg-[#182231] py-24 overflow-hidden">
      ...
      <Container maxWidth="2xl" className="relative z-10">
          ...
          {sectors.map((sector, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative rounded-2xl border border-white/[0.08] bg-[#0C121C]/80 backdrop-blur-md p-5 flex items-center gap-3.5 transition-all duration-250 hover:border-white/[0.2] hover:bg-[#111826]/90 hover:-translate-y-0.5 shadow-sm"
              >
                <div className="w-10 h-10 rounded-xl bg-[#141C2B] border border-white/[0.08] text-white flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                  <span className="text-lg">{sector.emoji}</span>
                </div>

                <span className="text-xs sm:text-sm font-semibold text-[#CBD5E1] group-hover:text-white transition-colors leading-tight font-display">
                  {sector.name}
                </span>
              </motion.div>
            );
        ...
        <div className="mt-12 text-center max-w-2xl mx-auto p-5 rounded-2xl bg-[#0D1420] border border-white/[0.08] shadow-md">
          <p className="text-sm text-[#E2E8F0] font-medium leading-relaxed">
            ✨ Si buscas ahorrar tiempo, atender mejor a tus clientes u organizar tus ventas, <span className="text-[#93C5FD] font-bold">podemos ayudarte.</span>
          </p>
        </div>
      </Container>
    </Section>
